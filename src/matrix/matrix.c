             ...
             *error;
{
	...
	*error = -1;
    ...
    b = (double **) malloc ((unsigned int) rows * (unsigned int) sizeof (double *));
    if (b == 0) {
	errno = ENOMEM;
	*error = -1;
	return (matrix);
    ...
    p = (double *) malloc ((unsigned int) rows * cols * (unsigned int) sizeof (double));
    if (p == 0) {
	errno = ENOMEM;
	*error = -1;
	return (matrix);
    ...
    dmat      a;
{
	...
	l = (int *) malloc ((unsigned int) 2 * (a.ub1 - a.lb1 + 1) * (unsigned int) sizeof (int));
	if (l == 0) {
